...
(function() {
  $(function() {
    var char, dropStick, i, kanjiId, onResize, svg, yakman, _i, _ref;
    $('#start-button').click(function() {
      $('#start-button').hide();
      return $('#curtain').velocity({
        height: '0'
      }, 1500, "ease-in", (function(_this) {
        return function() {
          return $('#curtain').remove();
        };
      })(this));
    });
    yakman = Yakman.random();
    $('.tile').each(function(i) {
      return $(this).addClass(yakman.tiles[i]);
    });
    for (i = _i = 0, _ref = yakman.name.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      kanjiId = Kanji.toId(yakman.name[i]);
      svg = $('#kanji_' + kanjiId).html();
      char = $('<div></div>').addClass('char').addClass(kanjiId).html(svg);
    ...
    (onResize = function() {
      ...
      switch (yakman.name.length) {
        case 3:
          ...
          return chars.eq(2).css('right', '20%');
        case 4:
